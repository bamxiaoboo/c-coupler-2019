...
Decomp_info::Decomp_info(const char *decomp_name, int decomp_id, int host_comp_id, int grid_id, int num_local_cells, const int *cell_indexes_in_decomp, const char *annotation, bool registered)
{
	...
	EXECUTION_REPORT(REPORT_ERROR, host_comp_id, CoR_grid->get_is_sphere_grid(), "the grid \"%s\" for registering parallel decomposition \"%s\" is not a H2D grid. Please check the model code related to the annoation \"%s\"",
    ...
    if (num_local_cells < 0) {
        EXECUTION_REPORT(REPORT_LOG, host_comp_id, true, "generate a dynamic decomposition \"%s\" (the corresponding annotation is \"%s\"", decomp_name);
