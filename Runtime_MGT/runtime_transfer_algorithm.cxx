...
{
    char local_line[NAME_STR_SIZE * 16], remote_line[NAME_STR_SIZE * 16];
    FILE *fp_cfg;
	...
	for(i = 0; i < num_transfered_fields; i ++) {
		...
		get_next_attr(comp_name, &local_line_p);
		get_next_attr(field_name, &local_line_p);
		...
		EXECUTION_REPORT(REPORT_ERROR, words_are_the_same(field_name, remote_field_name), 
			             ...
			             field_name, transfer_fields_cfg_file, remote_field_name);
		EXECUTION_REPORT(REPORT_ERROR, words_are_the_same(field_local_decomp_names[i], remote_field_remote_decomp_names) && words_are_the_same(field_remote_decomp_names[i], remote_field_local_decomp_name), 
			             ...
			             field_name, remote_field_name, transfer_fields_cfg_file);
