...
{
	EXECUTION_REPORT(REPORT_ERROR,-1, words_are_the_same(procedure_stage, PROCEDURE_STAGE_INITIALIZE) || words_are_the_same(procedure_stage, PROCEDURE_STAGE_KERNEL) ||
	...
	if (!words_are_the_same(compset_communicators_info_mgr->get_running_case_mode(), "initial") && words_are_the_same(procedure_stage, PROCEDURE_STAGE_INITIALIZE)) {
		EXECUTION_REPORT(REPORT_LOG,-1, true, "bypass all algorithms except model algorithms in initialize stage %s in restart run", procedure_name);
		for (int i = 0; i < runtime_algorithms.size(); i ++) {
			if (words_are_the_same(runtime_algorithms[i]->algorithm_type, "normal")) {
				...
				if (((Runtime_common_algorithm*) runtime_algorithms[i]->runtime_algorithm_object)->is_model_algorithm()) {
					EXECUTION_REPORT(REPORT_LOG,-1, true, "run model algorithm %s\n", runtime_algorithms[i]->algorithm_cfg_name);
					EXECUTION_REPORT(REPORT_LOG,-1, true, "before run algorithm %d in procedure %s", i, procedure_name);
					runtime_algorithms[i]->runtime_algorithm_object->run(words_are_the_same(procedure_stage, PROCEDURE_STAGE_KERNEL));
					EXECUTION_REPORT(REPORT_LOG,-1, true, "after run algorithm %d in procedure", i, procedure_name);
				}
			}
		}
		return;
    ...
    if (words_are_the_same(procedure_stage, PROCEDURE_STAGE_INITIALIZE)) {
        EXECUTION_REPORT(REPORT_ERROR,-1, timer_mgr->get_current_num_time_step() <= 1,
		...
		EXECUTION_REPORT(REPORT_ERROR,-1, !has_executed, 
	...
	EXECUTION_REPORT(REPORT_LOG,-1, true, "execute runtime procedure %s",  procedure_name);

    for (int i = 0; i < runtime_algorithms.size(); i ++) {
		if (runtime_algorithms[i]->runtime_algorithm_object == NULL) {
			EXECUTION_REPORT(REPORT_LOG,-1, true, "generate %s algorithm %s with id %d in procedure %s", runtime_algorithms[i]->algorithm_type, runtime_algorithms[i]->algorithm_cfg_name, i, procedure_name);
			...
			else EXECUTION_REPORT(REPORT_ERROR,-1, false, "error type of runtime algorithm %s", runtime_algorithms[i]->algorithm_type);
			EXECUTION_REPORT(REPORT_LOG,-1, true, "Finish generating algorithm %s with id %d", runtime_algorithms[i]->algorithm_cfg_name, i);
		}
...
{
    EXECUTION_REPORT(REPORT_LOG,-1, true, "runtime process manager write restart fields\n");

	EXECUTION_REPORT(REPORT_LOG,-1, true, "runtime process manager find kernel procedure for writing restart fields\n");

    for (int i = 0; i < runtime_algorithms.size(); i ++)
		if (runtime_algorithms[i]->runtime_algorithm_object != NULL)
            if (words_are_the_same(runtime_algorithms[i]->algorithm_type, "averaging")) {
				EXECUTION_REPORT(REPORT_LOG,-1, true, "runtime process manager find averaging algorithm for writing restart fields\n");
...
void Runtime_process_mgt::add_runtime_procedures(const char *procedures_cfg_file)
{
    ...
    if (!words_are_the_same(procedures_cfg_file, "NULL")) {
        ...
        while(get_next_line(line, tmp_cfg_fp)) {
            ...
            EXECUTION_REPORT(REPORT_ERROR,-1, end_algorithm_id >= start_algorithm_id, 
                         "the start algorithm id can not be larger than the end algorithm id in coupling procedure %s of component %s\n", 
                         procedure_name, compset_communicators_info_mgr->get_current_comp_name());
            EXECUTION_REPORT(REPORT_ERROR,-1, start_algorithm_id >= -1, 
			...
			if (start_algorithm_id == -1)
				EXECUTION_REPORT(REPORT_ERROR,-1, end_algorithm_id == -1, 
							 "procedure %s of component %s is an empty procedure, its start algorithm id and end algorithm id must be -1", 
							 procedure_name, compset_communicators_info_mgr->get_current_comp_name());
            EXECUTION_REPORT(REPORT_ERROR,-1, end_algorithm_id < (int)runtime_algorithms.size() && start_algorithm_id < (int)runtime_algorithms.size(), 
...
Runtime_procedure_mgt *Runtime_process_mgt::search_coupling_procedure(const char *procedure_name)
{
    ...
    EXECUTION_REPORT(REPORT_WARNING, -1, true, "coupling procedure %s has not been defined in component %s\n",
                 procedure_name, compset_communicators_info_mgr->get_current_comp_name());
...
void Runtime_process_mgt::execute_coupling_procedure(const char *procedure_name, const char *procedure_stage)
{
	...
	else EXECUTION_REPORT(REPORT_ERROR,-1, false, "WARNING: coupling procedure %s has not been set in the corresponding configuration file\n", procedure_name);
}
...
Runtime_process_mgt::~Runtime_process_mgt()
{
    ...
    for (int i = 0; i < runtime_algorithms.size(); i ++) {
        EXECUTION_REPORT(REPORT_LOG,-1, true, "before deleting %s algorithm", runtime_algorithms[i]->algorithm_type);
        if (words_are_the_same(runtime_algorithms[i]->algorithm_type, "remap"))
        ...
        EXECUTION_REPORT(REPORT_LOG,-1, true, "after deleting %s algorithm\n", runtime_algorithms[i]->algorithm_type);
    }
}
