...
Import_direction_setting::Import_direction_setting(int interface_id, TiXmlElement *redirection_element, const char *XML_file_name, std::vector<const char*> &interface_fields_name, int *fields_count)
{
	Inout_interface *interface_ptr = inout_interface_mgr->get_interface(interface_id);
	TiXmlElement *fields_element = NULL, *components_element = NULL, *remapping_element = NULL, *merge_element = NULL;
	int i;

	
	strcpy(this->interface_name, interface_ptr->get_interface_name());
	for (TiXmlNode *detailed_element_node = redirection_element->FirstChild(); detailed_element_node != NULL; detailed_element_node = detailed_element_node->NextSibling()) {
		...
		if (words_are_the_same(detailed_element->Value(), "fields")) {
			EXECUTION_REPORT(REPORT_ERROR, interface_ptr->get_comp_id(), fields_element == NULL, "When setting the redirection configuration of the import interface \"%s\" in the XML file \"%s\", the attribute of \"fields\" has been set at least twice in a redirection specification. Please verify.", interface_name, XML_file_name);
			fields_element = detailed_element;
			const char *default_str = detailed_element->Attribute("default");
			EXECUTION_REPORT(REPORT_ERROR, interface_ptr->get_comp_id(), default_str != NULL, "When setting the redirection configuration of the import interface \"%s\" in the XML file \"%s\", please set the value of \"default\" for the attribute of \"fields\"", interface_name, XML_file_name);
			EXECUTION_REPORT(REPORT_ERROR, interface_ptr->get_comp_id(), words_are_the_same(default_str, "off") || words_are_the_same(default_str, "all") || words_are_the_same(default_str, "remain"), "When setting the redirection configuration of the import interface \"%s\" in the XML file \"%s\", the value of \"default\" for the attribute of \"fields\" is wrong (legal values are \"off\", \"all\" and \"remain\"). Please verify.", interface_name, XML_file_name);
			if (words_are_the_same(default_str, "off")) {
				...
				for (TiXmlNode *field_element_node = detailed_element->FirstChild(); field_element_node != NULL; field_element_node = field_element_node->NextSibling()) {
					...
					EXECUTION_REPORT(REPORT_ERROR, interface_ptr->get_comp_id(), words_are_the_same(field_element->Value(),"field"), "When setting the attribute \"fields\" for the redirection configuration of the import interface \"%s\" in the XML file \"%s\", please use the keyword \"field\" for the name of a field", interface_name, XML_file_name);
					const char *field_name = field_element->Attribute("name");
					EXECUTION_REPORT(REPORT_ERROR, interface_ptr->get_comp_id(), field_name != NULL, "When setting the attribute \"fields\" for the redirection configuration of the import interface \"%s\" in the XML file \"%s\", please set the value of \"name\" for a field", interface_name, XML_file_name);
					char *field_name_str = new char [NAME_STR_SIZE];
					strcpy(field_name_str, field_name);	
					check_and_verify_name_format_of_string_for_XML(interface_ptr->get_comp_id(), field_name_str, "the field", XML_file_name);
					EXECUTION_REPORT(REPORT_ERROR, interface_ptr->get_comp_id(), fields_info->search_field_info(field_name) != NULL, "When setting the attribute \"fields\" for the redirection configuration of the import interface \"%s\" in the XML file \"%s\", an illegal field name (\"%s\") is detected. Please verify", interface_name, XML_file_name, field_name);
					...
					else EXECUTION_REPORT(REPORT_WARNING, interface_ptr->get_comp_id(), true, "When setting the attribute \"fields\" for the redirection configuration of the import interface \"%s\" in the XML file \"%s\", the interface does not contain a field with the name of \"%s\"", interface_name, XML_file_name, field_name);
				}				
			}
			else if (words_are_the_same(default_str, "all")) {
				fields_default_setting = 1;
				for (i = 0; i < interface_fields_name.size(); i ++) {
					EXECUTION_REPORT(REPORT_ERROR, interface_ptr->get_comp_id(), fields_count[i] == 0, "When setting the redirection configuration of the import interface \"%s\" in the XML file \"%s\", the configuration information of field \"%s\" has been set more than once. This is not allowed. Please note that the default value \"all\" means all fields. Please verify", interface_name, XML_file_name); 
					fields_count[i] ++;
					fields_name.push_back(interface_fields_name[i]);					
				}
			}
			...
			EXECUTION_REPORT(REPORT_ERROR, interface_ptr->get_comp_id(), components_element == NULL, "When setting the redirection configuration of the import interface \"%s\" in the XML file \"%s\", the attribute of \"components\" has been set at least twice in a redirection specification. Please verify.", interface_name, XML_file_name);
			components_element = detailed_element;
			const char *default_str = detailed_element->Attribute("default");
			EXECUTION_REPORT(REPORT_ERROR, interface_ptr->get_comp_id(), default_str != NULL, "When setting the redirection configuration of the import interface \"%s\" in the XML file \"%s\", please set the value of \"default\" for the attribute of \"components\"", interface_name, XML_file_name);
			EXECUTION_REPORT(REPORT_ERROR, interface_ptr->get_comp_id(), words_are_the_same(default_str, "off") || words_are_the_same(default_str, "all"), "When setting the redirection configuration of the import interface \"%s\" in the XML file \"%s\", the value of \"default\" for the attribute of \"componets\" is wrong (legal values are \"off\" and \"all\"). Please verify.", interface_name, XML_file_name);
			if (words_are_the_same(default_str, "off")) {
				...
				for (TiXmlNode *component_element_node = detailed_element->FirstChild(); component_element_node != NULL; component_element_node = component_element_node->NextSibling()) {
					...
					EXECUTION_REPORT(REPORT_ERROR, interface_ptr->get_comp_id(), words_are_the_same(component_element->Value(),"component"), "When setting the attribute \"components\" for the redirection configuration of the import interface \"%s\" in the XML file \"%s\", please use the keyword \"component\" for the full name of a component", interface_name, XML_file_name);
					const char *full_name = component_element->Attribute("full_name");
					EXECUTION_REPORT(REPORT_ERROR, interface_ptr->get_comp_id(), full_name != NULL, "When setting the attribute \"components\" for the redirection configuration of the import interface \"%s\" in the XML file \"%s\", please set the value of \"full_name\" for a component", interface_name, XML_file_name);
					char *full_name_str = new char [NAME_STR_SIZE];
					strcpy(full_name_str, full_name);				
					check_and_verify_name_format_of_string_for_XML(interface_ptr->get_comp_id(), full_name_str, "the the component model", XML_file_name);
			...
			EXECUTION_REPORT(REPORT_ERROR, interface_ptr->get_comp_id(), false, "When setting the redirection configuration of the import interface \"%s\" in the XML file \"%s\", the attribute of \"remapping_setting\" is not supported currently", interface_name, XML_file_name);
		}
		else if (words_are_the_same(detailed_element->Value(), "merge_setting")) {
			EXECUTION_REPORT(REPORT_ERROR, interface_ptr->get_comp_id(), false, "When setting the redirection configuration of the import interface \"%s\" in the XML file \"%s\", the attribute of \"merge_setting\" is not supported currently", interface_name, XML_file_name);
		}
		else EXECUTION_REPORT(REPORT_ERROR, interface_ptr->get_comp_id(), false, "When setting the redirection configuration of the import interface \"%s\" in the XML file \"%s\", \"%s\" is not a legal attribute. Please verify.", interface_name, XML_file_name, detailed_element->Value());
	}		
}
...
Import_interface_configuration::Import_interface_configuration(int interface_id, TiXmlElement *interface_element, const char *XML_file_name)
{
	...
	const char *interface_name = interface_ptr->get_interface_name();
	...
	for (TiXmlNode *redirection_element_node = interface_element->FirstChild(); redirection_element_node != NULL; redirection_element_node = redirection_element_node->NextSibling()) {
		...
		if (words_are_the_same(status, "off"))
			continue;
		import_directions.push_back(new Import_direction_setting(interface_id, redirection_element, XML_file_name, fields_name, fields_count));
	}
}


Component_import_interfaces_configuration::Component_import_interfaces_configuration(int comp_id)
{
	...
	sprintf(XML_file_name, "%s/CCPL_configs/%s.import.redirection.xml", comp_comm_group_mgt_mgr->get_root_working_dir(), comp_comm_group_mgt_mgr->get_global_node_of_local_comp(comp_id, "in Component_import_interfaces_configuration")->get_full_name());
	tmp_file = fopen(XML_file_name, "r");
	if (tmp_file == NULL) {
		EXECUTION_REPORT(REPORT_PROGRESS, comp_id, true, "As there is no import interface configuration file (the file name should be \"%s.import.redirection.xml\") specified for the component \"%s\", the coupling procedures of the import/export interfaces of this component will be generated automatically", 
	...
	EXECUTION_REPORT(REPORT_ERROR, comp_id, words_are_the_same(root_XML_element->Value(),"component_import_interfaces_configuration"), "The root XML element of the XML configuration file \"%s\" should be named \"component_import_interfaces_configuration\". Please verify.", XML_file_name);
	for (TiXmlNode *interface_XML_element_node = root_XML_element->FirstChild(); interface_XML_element_node != NULL; interface_XML_element_node = interface_XML_element_node->NextSibling()) {
		...
		EXECUTION_REPORT(REPORT_ERROR, comp_id, words_are_the_same(interface_XML_element->Value(),"import_interface"), "The XML element for specifying the configuration information of an import interface in the XML configuration file \"%s\" should be named \"import_interface\". Please verify.", XML_file_name);
		const char *interface_name = interface_XML_element->Attribute("name");
		EXECUTION_REPORT(REPORT_ERROR, comp_id, interface_name != NULL, "The format of the XML configuration file \"%s\" is wrong: the \"name\" of some import interface is not specified.", XML_file_name);
		char local_interface_name[NAME_STR_SIZE];
		strcpy(local_interface_name, interface_name);
		check_and_verify_name_format_of_string_for_XML(comp_id, local_interface_name, "the import interface", XML_file_name);
		Inout_interface *import_interface = inout_interface_mgr->search_an_interface(comp_id, interface_name);
		if (import_interface == NULL) {
			EXECUTION_REPORT(REPORT_WARNING, comp_id, true, "The redirection configuration of the import interface named \"%s\" has been specified in the XML configuration file \"%s\", while the component \"%s\" does not register the corresponding import interface. So this redirection configuration information is negleted.\"", 
				             interface_name, XML_file_name, comp_comm_group_mgt_mgr->get_global_node_of_local_comp(comp_id, "in Component_import_interfaces_configuration")->get_comp_name());
			continue;
		}
		EXECUTION_REPORT(REPORT_ERROR, comp_id, import_interface->get_import_or_export() == 0, "The redirection configuration of the import interface named \"%s\" has been specified in the XML configuration file \"%s\", while the component \"%s\" registers \"%s\" as an export interface. Please verify the model code with the annotation \"%s\" or the XML file",
			             interface_name, XML_file_name, comp_comm_group_mgt_mgr->get_global_node_of_local_comp(comp_id, "in Component_import_interfaces_configuration")->get_comp_name(),
			             interface_name, annotation_mgr->get_annotation(import_interface->get_interface_id(),"registering interface"));
		import_interfaces_configuration.push_back(new Import_interface_configuration(import_interface->get_interface_id(), interface_XML_element, XML_file_name));
	}
}
...
void Coupling_generator::generate_coupling_procedures()
{
	...
	inout_interface_mgr->write_all_interfaces_fields_info();
...
{
	Dictionary<int> *import_field_index_lookup_table = new Dictionary<int>(1024);
	Dictionary<int> *export_field_index_lookup_table = new Dictionary<int>(1024);
			...
			for (int i = 0; i < num_fields; i ++) {
				...
				if (import_or_export == 0) {
					if (import_field_index_lookup_table->search(field_name, false) == 0) {
						...
						import_fields_src_components[field_index].push_back(comp_full_name);
					}
				}
				else {
					if (export_field_index_lookup_table->search(field_name, false) == 0) {
						...
						export_fields_dst_components[field_index].push_back(comp_full_name);
					}
				}
			}
		}
